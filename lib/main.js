...
class LaunchChecklist {
	...
	run(key) {
		if( ! key ) {
			return this.runAll();
		} else if( ! this._tests.hasOwnProperty(key) ) {
			return utils.error(`'${key}' is not a valid test suite.`);
		}
	...
	runAll() {
		utils.success([
			'',
			'='.repeat(50),
			'Beginning WordPress Launch Checklist',
			'='.repeat(50),
			'',
		].join('\n'));

		for(const key in this._tests) {
			this._tests[key] = this.run(key);
		}
		return this;
	}
...
// exports = module.exports = LaunchChecklist;
